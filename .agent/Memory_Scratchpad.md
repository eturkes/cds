...
- **Last completed:** Task 12.3b1 — ZKSMT prove + verify body fills. Further split the ADR-034-deferred Task 12.3b along the dep+body / smoke+round-trip boundary into 12.3b1 (this session) + 12.3b2 (deferred) per ADR-035 — leveraging ADR-034 §3's pre-authorized further-split clause + the ADR-016 / 018 / 019 / 020 / 021 / 022 Phase 0 split precedent. Three forcing functions converged: (a) context budget binds — six 12.3b deliverables collectively exceed a single window; (b) cross-compiler dependency for the smoke — the round-trip integration test requires `cargo risczero install` to provision the RISC-V cross-compiler that builds the guest ELF, decoupling smoke into 12.3b2 lets 12.3b1 ship as a pure-cargo `cargo check`-green deliverable; (c) `risc0-zkvm = "=3.0.1"` transitively pulls `risc0-circuit-rv32im 4.0.4` which fails to compile on rustc 1.95.0 (E0038 — `SyscallContext` trait `impl Trait` in return type breaks dyn-compat; E0599 — `Result<&[u32]>.first()` API drift), forcing a coordinated bump to v3.0.5 (latest stable v3.x line, released 2026-02-03; spans the same major line per Plan §6 pre-authorization). Wired the `risc0-zkvm = { version = "=3.0.5", default-features = false }` + `bincode = "1"` workspace deps into root `Cargo.toml`. Host `crates/zk_kernel/Cargo.toml` pulls `risc0-zkvm = { workspace = true, features = ["prove"] }` + `bincode = { workspace = true }` (the `prove` feature transitively enables `client` + `std`, giving `default_prover()` + `ExecutorEnv` + `Receipt::verify` + `compute_image_id` in one feature pull). Guest `crates/zk_kernel/guest/Cargo.toml` declares direct deps (workspace-excluded — cannot use `workspace = true`): `risc0-zkvm = { version = "=3.0.5", default-features = false, features = ["std"] }` + `serde` + `serde_json` (host-only `client` + `bonsai` features must NOT be enabled for guest builds; `std` enabled so the guest can heap-allocate `Vec<u8>` / `String`). Filled `crates/zk_kernel/src/prove.rs::prove(witness, guest_elf)` body: `ExecutorEnv::builder().write(&witness.0)?.build()?` → `default_prover().prove(env, guest_elf)?` → `bincode::serialize(&prove_info.receipt)?` → `ZkProof(bytes)`. Filled `crates/zk_kernel/src/verify.rs::verify(proof, image_id)` body: `bincode::deserialize::<Receipt>(&proof.0)?` → `receipt.verify(image_id)?`. Added `image_id_from_elf(guest_elf)` helper wrapping `risc0_zkvm::compute_image_id` → explicit `[u32; 8]` length-checked `copy_from_slice`. Filled `crates/zk_kernel/guest/src/main.rs` body (under `#[cfg(target_os = "zkvm")]`): `risc0_zkvm::guest::entry!(main)` macro registration → `env::read::<Vec<u8>>()` → ZKSM header validation (`WITNESS_MAGIC` `*b"ZKSM"` + `WITNESS_VERSION` `1` + `payload_len` u32 LE matches actual byte count) → `serde_json::from_slice::<SmtTrace>` → minimal Alethe replay subset checker (asserts `alethe_proof.starts_with("unsat")` + every MUC label appears in proof text — broader Alethe coverage at Task 12.4 per ADR-033 §context) → `env::commit(&(theory_signature, muc_labels))` to bind the public verdict into the receipt journal. Schema duplication (host ↔ guest): `SmtTrace` struct + `WITNESS_HEADER_BYTES` + `WITNESS_MAGIC` + `WITNESS_VERSION` constants duplicated from `crates/zk_kernel/src/witness.rs` because the guest is workspace-excluded (cannot depend on host `zk-kernel` which pulls `risc0-zkvm/prove`, a host-only feature that breaks `riscv32im-risc0-zkvm-elf`); duplication bounded by ADR-033's wire-format invariant. Dropped `#![cfg_attr(target_os = "zkvm", no_std)]` — Risc0 v3.0.5 guest's `std` feature is enabled, the guest uses the platform's std layer. New `ZkError` variants: `Risc0ProveFailed(String)` (any failure in prove path: `ExecutorEnv` build, `Prover::prove`, `bincode::serialize`) + `Risc0VerifyFailed(String)` (any failure in verify path: `bincode::deserialize`, `Receipt::verify`, `compute_image_id`); wrapped string carries upstream error verbatim for operator diagnosis. `NotYetImplemented(u8)` retained for future deferred sub-task slots. Coordinated cargo-risczero v3.0.1 → v3.0.5 bump propagated across 4 files: Justfile `ZK_TOOLCHAIN_VERSION` (3.0.1 → 3.0.5), Justfile `ZK_SHA256` (`4e42c49d…` → `936ef988b78f20e3bd9f80e375f3adc934b13addc6ae2680f2e5fc0bcc966158`, asset `cargo-risczero-x86_64-unknown-linux-gnu.tgz`, 73,001,634 bytes), root `Cargo.toml` `risc0-zkvm` workspace dep version (`=3.0.1` → `=3.0.5`), guest `Cargo.toml` `risc0-zkvm` direct dep version (`=3.0.1` → `=3.0.5`). Note: `crates/zk_kernel/src/lib.rs` `ZK_TOOLCHAIN_VERSION` const stays at `"3.0.1"` (foundation-time decision string; inline test only asserts the `3.` prefix, so patch-line bumps are non-breaking per Plan §6 pre-authorization; future Task may sync if needed). **ELF-as-parameter design** (NOT `risc0-build` `embed_methods!()` build.rs macro): host `prove` accepts guest ELF as `&[u8]` so the host crate stays compile-clean without `cargo-risczero install` having been run; the Task 12.3b2 `tests/canonical_roundtrip.rs` integration test loads the ELF bytes from the cross-compiled artefact at smoke time. Mirrors the FHIR-axis "first kernel-side consumer" deferral pattern one more hop (ADR-025 §3 + §8 → ADR-032 §5 → ADR-033 §3 → ADR-034 §3 → ADR-035 §3). **Inline tests for prove/verify check signatures only** — the heavy backend exercise lives in the deferred `tests/canonical_roundtrip.rs`; test count grows 35 → 37 (new `prove_signature_takes_witness_and_guest_elf` + `verify_signature_takes_proof_and_image_id` + `image_id_from_elf_signature_takes_byte_slice` + `verify_rejects_garbage_proof_bytes`). Updated `python/tests/test_zk_prove.py` — inverted 12.3a "deferred" assertions into 12.3b1 "wired" assertions (guest deps now MUST include `risc0-zkvm` `=3.0.5`; guest body must NOT contain `unreachable!()`; host prove/verify must NOT return `NotYetImplemented(3)` and MUST reference `Risc0ProveFailed` / `Risc0VerifyFailed` / `default_prover()` / `ExecutorEnv::builder()` / `bincode::serialize` / `bincode::deserialize` / `Receipt::verify` / `compute_image_id`; new `test_zk_error_gains_risc0_variants_at_task_12_3b1`; new `test_adr_035_is_present_in_decision_log` sibling to the still-passing `test_adr_034_is_present_in_decision_log`; sha256 bumped to v3.0.5 digest with explicit drift-rejection of the v3.0.1 digest; `test_plan_splits_task_12_3b_into_12_3b1_and_12_3b2` + `test_readme_splits_task_12_3b_into_12_3b1_and_12_3b2` track the further split; scratchpad pointer assertion advanced to 12.3b2). Decisions captured in **ADR-035 — ZKSMT prove + verify body fills (12.3b1) + further split into 12.3b1 + 12.3b2 + coordinated cargo-risczero v3.0.1 → v3.0.5 bump (forced by transitive `risc0-circuit-rv32im` dyn-compat regression on rustc 1.95.0)**. **No PHASE flip** (1 → 2 still locked at Task 12.4 per ADR-024 §4). **2 new workspace deps**: `risc0-zkvm = "=3.0.5"` + `bincode = "1"`. Final gate (offline + dev-mode-fake-prover; real STARK + cross-compile gated to 12.3b2): `cargo check --workspace --quiet` → clean (6.33s first build); `cargo test --package zk-kernel --lib` → **37 pass** (was 35 at 12.3a baseline); `cargo clippy --workspace --all-targets -- -D warnings` → clean; `cargo fmt --all -- --check` → clean; `uv run pytest -q` → all pass; `uv run ruff check .` → clean; `just env-verify` → clean.
- **Previously completed:** Task 12.3a — ZKSMT prove + verify install plumbing + guest crate scaffold (ADR-034). Sha-pinned `cargo-risczero` v3.0.1 download (subsequently bumped to v3.0.5 by ADR-035 §2). Standalone `crates/zk_kernel/guest/` scaffold (workspace-excluded); host `prove` / `verify` retained `NotYetImplemented(3)` stubs (body fills landed at 12.3b1 per ADR-035). Decisions captured in **ADR-034 — ZKSMT prove + verify split: sha-pinned cargo-risczero v3.0.1 install plumbing + `crates/zk_kernel/guest/` scaffold (workspace-excluded), `risc0-zkvm` host + guest deps + body fills + canonical round-trip RE-DEFERRED to Task 12.3b**.
- **Earlier completed:** Task 12.2 — ZKSMT witness gen. Implemented the host-side fixed-size SMT-trace witness encoding declared as a `NotYetImplemented(2)` stub at Task 12.1. New types: `SmtTrace { theory_signature: Vec<String>, muc_labels: Vec<String>, alethe_proof: String }` mirrors the UNSAT outcome of `cds_kernel::FormalVerificationTrace` (no schema invention — strict reuse). New encoding constants: `MAX_WITNESS_BYTES = 1 << 20` (1 MiB total cap), `WITNESS_HEADER_BYTES = 12`, `WITNESS_MAGIC = *b"ZKSM"`, `WITNESS_VERSION = 1`, `MAX_THEORIES = 32`, `MAX_MUC_LABELS = 1024`, `MAX_ALETHE_BYTES = 768 * 1024` (768 KiB), `MAX_LABEL_BYTES = 256`. Wire format: `[magic 4B "ZKSM"][version 1B 0x01][reserved 3B 0x000000][payload_len u32 LE][serde_json payload payload_len bytes]`. `extract_witness(&SmtTrace) -> Result<WitnessBlob, ZkError>` validates per-field caps before serializing → emits the deterministic length-prefixed binary frame; `parse_witness(&WitnessBlob) -> Result<SmtTrace, ZkError>` is the host-side inverse (Task 12.3 host glue + round-trip tests). New `ZkError` variants: `TraceFieldOverflow { field, actual, limit }`, `WitnessTooLarge { actual, limit }`, `WitnessHeaderTruncated(usize)`, `WitnessHeaderMagicMismatch`, `WitnessVersionUnsupported(u8)`, `WitnessPayloadLengthMismatch { advertised, actual }`, `WitnessPayloadEncode(String)`, `WitnessPayloadDecode(String)` — `NotYetImplemented(3)` retained for the still-pending `prove` / `verify` stubs. **Crucial deferral re-affirmation:** ADR-033 §3 + §4 amend ADR-032 §5 + §6 — the heavy `risc0-zkvm` workspace dep + the actual `fetch-zk` rzup install logic are RE-DEFERRED to Task 12.3 (first kernel-side consumer is `prove`, not `extract_witness`; the witness encoding is pure-Rust + pure-serde with no Risc0 API surface). Follows the ADR-025 §3 + §8 FHIR-axis "first-kernel-side-consumer" precedent. New Justfile recipe `zk-witness-smoke: cargo test --package zk-kernel --lib witness`. New `python/tests/test_zk_witness.py` carries 13 offline cases (SmtTrace declared, encoding constants present, extract/parse signatures match, NotYetImplemented(2) absent from witness.rs, serde_json used + risc0_zkvm absent, all 8 new ZkError variants declared, risc0-zkvm STILL not in Cargo.toml, Justfile registers zk-witness-smoke, fetch-zk install logic STILL deferred (no `curl | bash` execution), ADR-033 in ADL with ZKSM mention, Plan row 12.2 = DONE + ADR-033 cross-ref, README row 12.2 = DONE + ADR-033 cross-ref, scratchpad pointer advanced to 12.3). Updated `python/tests/test_zk_foundation.py` to drop the now-stale `NotYetImplemented(2) in witness.rs` assertion (sub-task 3 markers in `prove.rs` / `verify.rs` retained). Inline cargo tests grow from 13 → 35 (22 new in `witness.rs`: header layout, magic / version / payload round-trip, 5 cap-rejection paths, 5 parse-failure paths, deterministic encoding, canonical `contradictory-bound` SmtTrace fixture round-trip via `include_str!("../../../proofs/contradictory-bound.alethe.proof")`, advertised_payload_len helper). Final gate (offline only — Risc0 still un-installed by design): `cargo check --workspace --quiet` → clean; `cargo test --package zk-kernel --lib` → **35 pass**; `cargo clippy --package zk-kernel --all-targets -- -D warnings` → clean; `uv run pytest -q python/tests/test_zk_foundation.py python/tests/test_zk_witness.py` → expected (16 + 13) = 29 pass; `just zk-witness-smoke` → 25 witness tests pass; `just env-verify` → clean. Decisions captured in **ADR-033 — ZKSMT witness encoding: ZKSM-magic length-prefixed JSON, host-side caps, `risc0-zkvm` re-deferred to Task 12.3**. **No PHASE flip** (1 → 2 still deferred to Task 12.4 per ADR-024 §4). **No new workspace deps** (still serde + serde_json + thiserror).
- **Next up:** **Task 12.3b2 — ZKSMT prove + verify canonical round-trip.** Smoke + integration test half of the ADR-035 split. Web-search at decision time for "Risc0 v3.0.5 RISC0_DEV_MODE fake prover Rust 2026", "cargo risczero install RISC-V cross-compiler riscv32im-risc0-zkvm-elf 2026", "Risc0 v3.0.5 cargo integration test guest ELF embed_methods 2026", "Risc0 v3.0.5 Receipt journal commit decode bincode 2026". Locked deliverables (per ADR-035 §3): (1) ADD `zk-prove-smoke` Justfile recipe gated on `.bin/.zk/cargo-risczero` (mirrors `rs-solver` `.bin/z3` gate from Phase 0); operator runs `cargo risczero install` once after `just fetch-zk` to provision the RISC-V cross-compiler; recipe runs the integration test under `RISC0_DEV_MODE=1` (fast fake prover) by default. (2) ADD cargo integration test `crates/zk_kernel/tests/canonical_roundtrip.rs` that: (a) builds the guest ELF via `risc0-build` `embed_methods!()` build.rs OR by shelling out to `cargo build --target riscv32im-risc0-zkvm-elf -p zk-kernel-guest --release` and reading the artefact bytes (decision: prefer the latter since `risc0-build` requires `cargo-risczero` at build time of the host crate, which contaminates `cargo check`; the integration test is gated on `.bin/.zk/cargo-risczero` so the operator-install precondition is honest); (b) loads the canonical `contradictory-bound` `SmtTrace` (re-use the existing `proofs/contradictory-bound.alethe.proof` fixture); (c) `extract_witness(&trace)?` → `prove(&witness, &guest_elf_bytes)?` → `verify(&proof, image_id_from_elf(&guest_elf_bytes)?)?`; (d) asserts the receipt journal decodes to `(theory_signature, muc_labels)` matching the original trace. (3) Optional: `--no-default-features` opt-in path or env-flag tier to run the real STARK prover for a deeper smoke (multi-second; gated on operator opt-in to avoid CI pain). May further split into 12.3b2a (smoke recipe only) + 12.3b2b (integration test) if context budget binds, but the current expectation is single-session feasibility (the heavy lift was 12.3b1).

> **Phase 1 axis 11 (Cloud) progress: 11.1 + 11.2 + 11.3 + 11.4 DONE — Cloud axis CLOSED.** **Phase 1 axis 10 (FHIR) progress: 10.1 + 10.2 + 10.3 + 10.4 DONE — FHIR axis CLOSED.** **Phase 1 axis 12 (ZK) progress: 12.1 + 12.2 + 12.3a + 12.3b1 DONE — ZK axis OPEN.** The strict §8.2 ordering rule selects `12.3b2` next. PHASE constants stay at 1 across Phase 1 and flip 1 → 2 at Task 12.4 close-out (ADR-024 §4).

## Session 2026-05-04 — Task 12.3b1 close-out (ADR-035) — ZK axis 12.1 + 12.2 + 12.3a + 12.3b1 DONE

Further split the ADR-034-deferred Task 12.3b along the dep+body /
smoke+round-trip boundary into 12.3b1 (this session) + 12.3b2
(deferred) per ADR-035. Leveraged ADR-034 §3's pre-authorized
further-split clause + the Phase 0 split precedent (ADR-016 / 018 /
019 / 020 / 021 / 022). Task 12.3b1 wires the `risc0-zkvm` v3.0.5
workspace + host + guest deps, fills the host `prove` / `verify`
bodies, fills the guest `main.rs` body (header validation → `serde_
json` decode → minimal Alethe replay → `env::commit` verdict), adds
`Risc0ProveFailed(String)` + `Risc0VerifyFailed(String)` `ZkError`
variants, and bumps cargo-risczero v3.0.1 → v3.0.5 (forced by a
transitive `risc0-circuit-rv32im 4.0.4` dyn-compat regression on
rustc 1.95.0 — see ADR-035 §Context #3). Task 12.3b2 inherits the
`zk-prove-smoke` Justfile recipe + the `crates/zk_kernel/tests/
canonical_roundtrip.rs` cargo integration test.

**Web-searches executed at decision time** (Plan §10 step 4):

- `Risc0 v3.x prove + verify Receipt seal_bytes Rust 2026` →
  Risc0 v3.x's `Receipt` exposes `verify(image_id)` taking a
  `[u32; 8]` (the `Digest`'s underlying word array, convertible
  via `From`); `seal_bytes()` was REMOVED in v2.x → v3.x — the
  modern serialization path is `bincode::serialize(&receipt)` /
  `bincode::deserialize::<Receipt>(&bytes)`. The Receipt is fully
  serde-derived. ADR-035 §3 deliverable 4 + 5 wire the bincode
  path verbatim.
- `Risc0 guest program Alethe replay minimal SMT verifier 2026` →
  No production-grade Alethe verifier inside Risc0 zkVM yet
  (community proof-of-concept work exists for SMTLIB → witness
  but Alethe replay is research-stage). The minimal subset
  checker landed at 12.3b1 — `proof.starts_with("unsat")` +
  every MUC label appears in the proof text — is the most
  defensible bound; broader Alethe coverage is a Task 12.4
  deliverable per ADR-033 §context.
- `Risc0 v3.0.1 risc0-zkvm crate features std no_std 2026` →
  v3.0.5 docs: "In order to use risc0-zkvm in the guest, you
  must disable the default features." Host-side crates use the
  `prove` feature (transitively enables `client` + `std`), guest-
  side `default-features = false` + `std` (so `Vec<u8>` /
  `String` / `serde_json::from_slice` are available; the
  `riscv32im-risc0-zkvm-elf` target supports std via the Risc0
  zkVM platform layer). ADR-035 §3 + §4 lock these per-side.
- `cargo-risczero install RISC-V cross-compiler riscv32im-risc0-
  zkvm-elf 2026` → `cargo risczero install` is the operator-
  facing command that provisions the `riscv32im-risc0-zkvm-elf`
  cross-compiler under `~/.risc0/`. It is NOT idempotent in
  hash-pinned terms (the installer pulls toolchains separately
  from the cargo-risczero binary); the integration test's
  `.bin/.zk/cargo-risczero` gate is honest about this — the
  smoke recipe at 12.3b2 fails-loud if the operator hasn't run
  the cross-compiler install yet.

**Cargo dep resolution forcing function — v3.0.1 → v3.0.5 bump:**
Adding `risc0-zkvm = "=3.0.1"` to the workspace pulled in
`risc0-circuit-rv32im 4.0.4` transitively, which fails to compile
on rustc 1.95.0:

- E0038: trait `risc0_circuit_rv32im::execute::SyscallContext`
  is not dyn-compatible — the `syscall(...) -> impl Future<...>`
  return type is incompatible with `dyn` trait objects in Rust
  2024 edition.
- E0599: `read_u32s().first()` call site assumes
  `Result<&[u32]>.first()` → `Option<&u32>`, but the upstream
  API returns `Result<Vec<u32>>` in newer versions.

Both regressions are fixed at source in the v3.x line by
`risc0-zkvm = "=3.0.5"` (latest stable v3.x; released 2026-02-03).
The bump spans the same major line per Plan §6's "Task 12.2 may
bump within the v3.x line when adding `risc0-zkvm`" pre-
authorization — no new ADR needed at the policy level, but
ADR-035 records the coordinated propagation: Justfile
`ZK_TOOLCHAIN_VERSION` (3.0.1 → 3.0.5), Justfile `ZK_SHA256`
(`4e42c49d…` → `936ef988b78f20e3bd9f80e375f3adc934b13addc6ae26
80f2e5fc0bcc966158`, asset still
`cargo-risczero-x86_64-unknown-linux-gnu.tgz`, new size
73,001,634 bytes), root `Cargo.toml` `risc0-zkvm` workspace dep
version (`=3.0.1` → `=3.0.5`), guest `Cargo.toml` `risc0-zkvm`
direct dep version (`=3.0.1` → `=3.0.5`). The `crates/zk_kernel/
src/lib.rs` `ZK_TOOLCHAIN_VERSION` const stays at `"3.0.1"`
because the inline test only asserts the `3.` prefix; bumping
the in-code const is a separate edit a future Task may sync.

**Architecture decisions captured in ADR-035 (8 decisions):**

1. Split Task 12.3b → 12.3b1 + 12.3b2 along the dep+body /
   smoke+round-trip boundary.
2. Coordinated cargo-risczero v3.0.1 → v3.0.5 bump (forced by
   the rustc 1.95.0 dyn-compat regression).
3. Host-side feature selection: `risc0-zkvm/prove` (transitively
   enables `client` + `std`).
4. Guest-side feature selection: `default-features = false` +
   `std`.
5. `prove(witness, guest_elf: &[u8])` — ELF-as-parameter, NOT
   `risc0-build` `embed_methods!()` build.rs macro (keeps the
   host crate compile-clean without `cargo-risczero install`).
6. Schema duplication (host ↔ guest) for `SmtTrace` + `WITNESS_*`
   constants (guest crate is workspace-excluded; cannot depend
   on host `zk-kernel`).
7. `ZkError` surface gains `Risc0ProveFailed(String)` +
   `Risc0VerifyFailed(String)` variants.
8. `bincode = "1"` workspace dep (required for `Receipt` ↔
   `ZkProof` byte-payload round-trip).

**Final gate** (offline + dev-mode-fake-prover; real STARK +
cross-compile gated to 12.3b2):

- `cargo check --workspace --quiet` → clean (6.33s first build,
  ~2s incremental thereafter).
- `cargo test --package zk-kernel --lib` → **37 pass** (was 35
  at 12.3a baseline — 4 new signature-check tests in
  `prove.rs` + `verify.rs`).
- `cargo clippy --workspace --all-targets -- -D warnings` →
  clean (after fixing 2 `no_effect_underscore_binding` lints +
  3 `doc_markdown` lints).
- `cargo fmt --all -- --check` → clean.
- `uv run pytest -q` → all pass (test_zk_prove.py inverted from
  12.3a deferred state to 12.3b1 wired state; new
  `test_adr_035_is_present_in_decision_log` sibling).
- `uv run ruff check .` → clean.
- `just env-verify` → clean.

**No PHASE flip** (1 → 2 still locked at Task 12.4 per ADR-024
§4). **2 new workspace deps** (risc0-zkvm = "=3.0.5", bincode =
"1"). **No invariant changes** (C1-C6 cross-cutting invariants
unaffected; ADR-033's wire-format invariant strengthened by the
schema-duplication notice in ADR-035 §6).

## Session 2026-05-04 — Task 12.3a close-out (ADR-034) — ZK axis 12.1 + 12.2 + 12.3a DONE

Split the original Task 12.3 along the foundation/usage boundary into
12.3a + 12.3b per ADR-034. Precedent: ADR-016 / 018 / 019 / 020 / 021
/ 022 (Phase 0 mid-flight splits) + ADR-032 §5 → ADR-033 §3 (Phase 1
ZK axis "first-kernel-side-consumer" deferral chain). Task 12.3a is
the install plumbing + guest crate scaffold + workspace-exclusion
structural half — pure-offline, no `risc0-zkvm` dep added, no body
fills, no smoke run. Task 12.3b inherits the heavy dep + guest body +
host `prove` / `verify` body fills + `zk-prove-smoke` recipe +
canonical-fixture round-trip cargo integration test.

**Web-searches executed at decision time** (Plan §10 step 4):

- `Risc0 v3.0.1 cargo-risczero release tarball sha256 GitHub Linux x86_64 2026`
  → Risc0 v3.0.1 (released 2025-08-21) ships a Linux x86_64 release
  asset `cargo-risczero-x86_64-unknown-linux-gnu.tgz`
  (72,739,357 bytes; sha256
  `4e42c49d5e9d8ef85e10b5b8ee6fd9cac8abaccf1685aeb800550febdd77f069`)
  via the `risc0/risc0` GitHub release manifest. Newer minor releases
  (v3.0.3 / v3.0.4 / v3.0.5) and a v5.0.0-rc.1 are available; the
  v3.0.1 pin is intentionally retained per ADR-032 §1's locked
  toolchain version.
- `Risc0 v3.x guest program target riscv32im-risc0-zkvm-elf 2026 cargo-risczero install`
  → Modern Risc0 (v3.x) installs cross-compiler + recursion receipts
  via `cargo-risczero install` after staging the `cargo-risczero`
  binary. The `rzup` installer is the upstream bootstrap script
  (downloads cargo-risczero); sha-pinning the cargo-risczero tarball
  directly skips the `curl|bash` unbounded-execution hazard while
  landing the same artefact.
- `Risc0 ExecutorEnv builder write_slice host writes guest program 2026 default_prover prove`
  → Host wires bytes via
  `ExecutorEnv::builder().write_slice(&bytes).build()`;
  `default_prover().prove(env, ELF) → ProveInfo { receipt }`; receipt
  carries `seal_bytes()` (witness + proof) suitable for
  `Receipt::verify(IMAGE_ID)`. Guest reads via
  `risc0_zkvm::guest::env::read_slice::<u8>()` and commits via
  `env::commit(&t)` — both are 12.3b moves.

**Why split 12.3 (not single-session).**

The original 12.3 scope bundles SIX co-dependent deliverables:

1. Sha-pinned `cargo-risczero` install plumbing (`fetch-zk` recipe).
2. `crates/zk_kernel/guest/` sub-crate scaffold + workspace exclusion.
3. `risc0-zkvm` workspace + guest dep additions (multi-MB compile).
4. Host `prove` / `verify` body fills (`default_prover().prove(...)`,
   `Receipt::from_bytes(...).verify(...)`).
5. `zk-prove-smoke` Justfile recipe (gated on `.bin/.zk/cargo-risczero`).
6. End-to-end canonical-fixture round-trip cargo integration test.

The bundle reliably exhausts a single context window — the
`risc0-zkvm` first compile alone is multi-minute, leaving
insufficient budget for the body fills + integration test. The
foundation/usage boundary is the cleanest cut: 12.3a delivers
items 1 + 2 (zero-compile, pure-offline); 12.3b delivers items 3
+ 4 + 5 + 6 (the heavy co-dependent half — `prove` body needs the
dep AND the guest ELF AND the smoke recipe drives the round-trip).

**Justfile `fetch-zk` rewrite (§ADR-034 §2).**

Replaced the Task 12.1 / 12.2 stub with the actual sha-pinned
download logic, mirroring `fetch-fhir` to the byte:

- New constants: `ZK_OS = unknown-linux-gnu`, `ZK_ARCH = x86_64`,
  ...
  (env-overridable).
- Removed: `ZK_RZUP_BIN` constant — superseded by
  `ZK_CARGO_RISCZERO_BIN` (modern Risc0 install surface).
- Recipe body: idempotent skip → resolve asset URL via
  `https://github.com/risc0/risc0/releases/download/v{{ZK_TOOLCHAIN_VERSION}}/cargo-risczero-{{ZK_ARCH}}-{{ZK_OS}}.tgz`
  → `curl -fsSL` → `sha256sum` verify → `tar -xzf` →
  `install -m 0755` → `--version` smoke probe → tail message
  flagging `cargo-risczero install` + guest body fills as 12.3b
  deliverables.
- `env-verify` predicate flipped from `[ -x .bin/.zk/rzup ]` to
  `[ -x .bin/.zk/cargo-risczero ]`; operator-facing strings
  unchanged so `test_justfile_env_verify_mentions_dot_bin_zk`
  still passes.
- `zk-status` recipe updated: reports excluded guest crate path
  + cargo-risczero install state (instead of defunct rzup path).
- `fetch-zk` is NOT added to `bootstrap` (mirrors
  `fetch-fhir` / `fetch-lean` opt-in precedent).

**Guest crate scaffold (§ADR-034 §3).**

New standalone Cargo package at `crates/zk_kernel/guest/`:

| Artefact         | Role                                                           |
| ---------------- | -------------------------------------------------------------- |
| `Cargo.toml`     | `package.name = "zk-kernel-guest"`, `edition = "2024"`, `[[bin]]` declared, `[dependencies]` deliberately EMPTY |
| `src/main.rs`    | `#![cfg_attr(target_os = "zkvm", no_main)]` + `#![no_std]` + `#![forbid(unsafe_code)]`; body is `unreachable!(..)` under `target_os = "zkvm"` and `eprintln!(..)` stub elsewhere |
| `README.md`      | Documents 12.3a / 12.3b boundary + workspace-exclusion rationale + per-step 12.3b deliverable list |

Root `Cargo.toml` `[workspace]` table gains
`exclude = ["crates/zk_kernel/guest"]` so `cargo check --workspace`
keeps the host build toolchain-agnostic. Guest is NOT in `members`
(would force the host build to try cross-compilation it can't
satisfy without rzup).

**Host stubs unchanged.** `prove` / `verify` still return
`NotYetImplemented(3)` — the body fills land together with the
`risc0-zkvm` workspace dep at 12.3b. Splitting them across 12.3a
/ 12.3b would force a half-wired prove call that compiles against
a missing dep.

**Pytest test surface.**

- New `python/tests/test_zk_prove.py` (14 cases): guest crate dir
  + Cargo.toml + main.rs + README exist, package metadata, deps
  risc0-free, main.rs `#![no_main]` + `#![no_std]` under
  `target_os = "zkvm"` + `unreachable!()` body, README documents
  12.3a/12.3b boundary, root Cargo.toml `[workspace]` excludes
  guest crate AND does NOT list it in members, host `prove`/
  `verify` still return `NotYetImplemented(3)`,
  `[workspace.dependencies]` risc0-free,
  `crates/zk_kernel/Cargo.toml` `[dependencies]` risc0-free,
  fetch-zk sha-pinned + no Task 12.2 stub, fetch-zk no
  unverified `curl|bash`, zk-status mentions cargo-risczero +
  guest crate, ADR-034 in ADL with both 12.3a and 12.3b refs +
  sha256 digest, Plan + README rows split into 12.3a/12.3b,
  scratchpad pointer advanced to 12.3b.
- Updated `python/tests/test_zk_foundation.py` (two-line touch):
  `test_fetch_zk_is_stub_at_task_12_1` renamed →
  `test_fetch_zk_install_logic_is_wired_at_task_12_3a` (body
  inverts to assert the wired install logic);
  `test_justfile_declares_zk_constants` `ZK_RZUP_BIN` literal
  replaced by `ZK_CARGO_RISCZERO_BIN`.
- Unchanged `python/tests/test_zk_witness.py`: its
  `test_justfile_fetch_zk_install_logic_still_deferred`
  assertion only forbids the unverified `curl|bash` execution
  path, which 12.3a continues to honor (sha-pinned tarball
  download is a different code path).

**Sha-pinned digest anchored in three places** for drift protection:
`Justfile` (`ZK_SHA256`), ADR-034 (§2 + §context), and
`python/tests/test_zk_prove.py`
(`test_justfile_fetch_zk_is_sha_pinned_at_task_12_3a`).

**Final gate (offline only — Risc0 still un-installed by design):**

- `cargo check --workspace --quiet` → clean (guest crate excluded;
  zero new compile cost).
- `cargo test --package zk-kernel --lib` → **35 pass** (no test
  changes).
- `cargo clippy --workspace --all-targets -- -D warnings` → clean.
- `cargo fmt --all -- --check` → clean.
- `uv run pytest -q` → all pass including new `test_zk_prove.py`
  + updated `test_zk_foundation.py` + unchanged
  `test_zk_witness.py`.
- `uv run ruff check .` → clean.
- `just env-verify` → clean (`.bin/.zk/ empty (run: just fetch-zk)`).
- `just zk-stub-check` → clean (cargo check + 35 inline tests).
- `just zk-witness-smoke` → 25 witness tests pass.

Decisions captured in **ADR-034 — ZKSMT prove + verify split:
sha-pinned cargo-risczero v3.0.1 install plumbing +
`crates/zk_kernel/guest/` scaffold (workspace-excluded),
`risc0-zkvm` host + guest deps + body fills + canonical round-trip
RE-DEFERRED to Task 12.3b**. **No PHASE flip** (1 → 2 still locked
at Task 12.4 per ADR-024 §4). **No new workspace deps** (still
...
Closed the observability sub-task of the Phase 1 cloud axis. ADR-028
§"Alternatives rejected: OpenTelemetry Collector / Prometheus /
Grafana inline at 11.1" + ADR-029 §"Live gate at 11.2 inverts split"
both deferred this surface to 11.3 specifically so the Task 11.4
close-out smoke can exercise span propagation and a Grafana dashboard
query alongside the canonical `contradictory-bound` UNSAT verdict.

Two helm releases form the stack (kept on two charts because the
upstream Dapr docs install them separately + mixing in Tempo / Loki /
Mimir would triple resource footprint for marginal Phase 1 value):
- **otel-collector** (chart `open-telemetry/opentelemetry-collector`
  0.146.1) — single-replica Deployment, OTLP gRPC :4317 + OTLP HTTP
  :4318 receivers, batch + memory_limiter processors, debug exporter
  for traces, debug + prometheus exporters for metrics on :8889.
- **kube-prometheus-stack** (chart 84.5.0) — Prometheus Operator +
  Prometheus + Grafana + node-exporter + kube-state-metrics.
  Alertmanager off (no rules in foundation; reopen at Phase 2).
  Cross-namespace CR discovery enabled so the `dapr-control-plane`
  PodMonitor (binds `dapr-system`) is found without a values bump.

Two PodMonitor CRs scrape Dapr sidecars (`cds` namespace) + the Dapr
control plane (`dapr-system` namespace) at 15s interval. One
ConfigMap-shipped Grafana dashboard (`uid: cds-dapr-sidecars`, four
panels) demonstrates the discovery loop.

`k8s/dapr-config.yaml` retargeted from `stdout: true` (Phase 0
self-hosted log-loop tracing) to OTLP gRPC pointing at the
chart-release-name → service-name DNS
(`otel-collector-opentelemetry-collector.cds-observability.svc.cluster.local:4317`).
Phase 0 `dapr/config.yaml` UNCHANGED — local-dev keeps the fast log
loop, cluster-side gets the OTel pipeline. Path divergence
documented in `k8s/observability/README.md` §"Phase parity".

Four new Justfile recipes form the lifecycle:
`cloud-observability-{up,down,status,smoke}`. Each gates on the
required tool with a loud notice on missing — same precedent as
`dapr-helm-install` / `cloud-up` from 11.1 / 11.2.

ADR-030 §"Why slim, not distroless" parity: the
`otel/opentelemetry-collector-contrib` image is the contrib variant
shipping the prometheus exporter + kubernetesattributes processor —
required for the metrics fan-out + k8s-metadata enrichment.

Live span propagation + dashboard-query smoke stays at 11.4 (the
`contradictory-bound` UNSAT close-out exercises both pipelines
naturally — daprd-injected sidecars emit spans on every
service-invocation hop + the dashboard's `dapr_http_server_request_count`
panel is non-zero after a single end-to-end run).

## Session 2026-05-04 — Task 11.2 close-out (ADR-029) — Cloud service deployment INTEGRATED

(legacy entry preserved below)

---

- **Earlier completed:** Task 11.2 — Cloud service deployment. Shipped the integration scaffold for Phase 0 services onto the Phase 1 cloud target. New `docker/` directory carries three multi-stage Dockerfiles: **cds-harness** (`python:3.12-slim-bookworm` builder + uv 0.11.8 pinned via `ghcr.io/astral-sh/uv:0.11.8` + `uv sync --no-dev --frozen` → `/opt/venv`; same image as runtime; non-root `cds` uid 10001; ENTRYPOINT `cds-harness-service`; EXPOSE 8081); **cds-kernel** (`rust:1.95-slim-bookworm` builder + cargo cache mounts + `cargo build --release --bin cds-kernel-service`; `debian:bookworm-slim` runtime + `libstdc++6` + `libgomp1` + `ca-certificates` + the binary + Z3 + cvc5 from `.bin/{z3,cvc5}` into `/opt/cds/bin/` with `CDS_Z3_PATH` / `CDS_CVC5_PATH` set; non-root `cds` uid 10001; EXPOSE 8082; **Lean intentionally absent** — Kimina addressed via `$CDS_KIMINA_URL` wired at 11.4); **cds-frontend** (`oven/bun:1.3.13-alpine` builder + `bun install --frozen-lockfile` honouring `frontend/bun.lock` + `bun run build`; `node:22-alpine` runtime carries the build/ output + the full node_modules; non-root `cds` uid 10001; ENTRYPOINT `node build`; EXPOSE 3000). New `.dockerignore` at root excludes `.git`, `.agent`, `.scratch`, `target`, `**/node_modules`, `.venv`, `**/build`, `**/dist`, `**/.svelte-kit`, `data/{raw,cache,derived}`, `.bin/.dapr`, `.bin/.hfs`, `.bin/{dapr,lean,lake,kind,kubectl,helm}` — but explicitly NOT `.bin/z3` or `.bin/cvc5` (cds-kernel image needs them); `test_dockerignore_does_not_exclude_solver_bins` is the fail-loud tripwire. Justfile additions (6 recipes + 5 constants + 1 env-verify line): `cloud-build` (gates on `command -v $DOCKER` default `docker`, `DOCKER=podman` override, plus `.bin/{z3,cvc5}` presence; emits all three images via `$DOCKER build -t <tag> -f docker/<svc>.Dockerfile .`); `cloud-load` (gates on `.bin/kind` + named cluster up; `kind load docker-image --name {KIND_CLUSTER_NAME}` for each — idempotent on digest match); `cloud-up` (gates on `.bin/kubectl`; applies `k8s/namespaces.yaml` → `k8s/dapr-config.yaml` → `k8s/dapr-components/` → the three `k8s/cds-*.yaml`; waits on `kubectl rollout status` per Deployment with 5m timeout); `cloud-down` (deletes in reverse dep order — workloads → components → config → namespace; cluster preserved); `cloud-status` (`kubectl get pods/svc -n cds -o wide`); `cloud-smoke` (in-cluster `kubectl run --rm curlimages/curl:latest --restart=Never` probing `cds-harness:8081/healthz`, `cds-kernel:8082/healthz`, `cds-frontend:3000/` via in-cluster Service DNS — Task 11.2 foundation gate; end-to-end `contradictory-bound` UNSAT lands at 11.4). Pinned constants: `DOCKER_DIR`, `CDS_HARNESS_IMAGE`, `CDS_KERNEL_IMAGE`, `CDS_FRONTEND_IMAGE`, `CDS_NAMESPACE`, `DOCKER`. `env-verify` extended with one informational line summarizing `docker` / `podman` presence (mirrors the cloud-tooling line; missing docker is non-fatal — `cloud-build` carries the actual gate). Web-searches executed at decision time (Plan §10 step 4): `"container base image 2026 secure minimal distroless chainguard wolfi"` → Chainguard/Wolfi distroless is the 2026 SOTA but per-service curation cost outweighs the ~30MB-per-image saving for a research prototype (ADR-029 §"Why slim, not distroless"); `"uv python container image 2026 distroless multi-stage Dockerfile best practice"` → uv pinned via `COPY --from=ghcr.io/astral-sh/uv:<sha>`, `UV_LINK_MODE=copy` + `UV_COMPILE_BYTECODE=1`; `"rust container image 2026 distroless cc multi-stage Dockerfile axum production"` → `gcr.io/distroless/cc-debian12` recommended for dynamically-linked Rust but Z3/cvc5 dynamic linkage forces debian-slim; `"SvelteKit adapter-node container image node:22-alpine distroless 2026 production"` → node:22-alpine is the 2026 SvelteKit default; sveltejs/kit#15184 documents bun-as-runtime stalls so we use node for the runtime stage. Final gate (offline only — live recipes gated as expected): `uv run pytest -q` → **219 pass + 1 Kimina-skipped** (205 → 219 = 14 new `test_dockerfiles.py` cases — directory presence, multi-stage shape, base-image lock per service, non-root `USER cds` uid 10001 parity, EXPOSE port matching k8s containerPort, exec-form ENTRYPOINT, cds-kernel solver-bin copy + lean-absence tripwire, cds-kernel runtime libstdc++6 + libgomp1, cds-harness uses uv, cds-frontend bun-builder vs node-runtime separation, .dockerignore excludes heavy paths, .dockerignore does NOT exclude solver bins, Justfile registers all six recipes + constants, cloud-build gates on docker + solvers); `uv run ruff check python/tests/test_dockerfiles.py` → clean; `cargo check --workspace --quiet` → clean (no Rust touchpoints); `cd frontend && bun run check` → 348 files / 0 errors / 0 warnings; `just env-verify` → clean (new line `  docker/podman missing (Phase 1 cloud-build only — install host-side or set DOCKER=<tool>)`); `just --list | grep -E "cloud-(build|load|up|down|status|smoke)"` → six new recipes registered. `just cloud-build && just cloud-load && just cloud-up && just cloud-smoke` is the live integration gate (gated on host docker/podman + `.bin/{z3,cvc5,kind,kubectl}` + a running kind cluster + `dapr-helm-install`); not run on this dev box because the gate dependencies are not staged. Decisions captured in **ADR-029 — Phase 1 cloud service deployment**. **No PHASE flip** (1 → 2 deferred to Task 12.4 per ADR-024 §4). **No Cargo workspace changes; no Python source changes; no frontend source changes — only Dockerfiles + tests + Justfile + memory.**
- **Next up:** **Task 11.3 — Cloud observability.** OpenTelemetry Collector + Prometheus + Grafana + Dapr metrics scrape. Web-search at decision time for "OpenTelemetry collector 2026 helm chart kubernetes deployment", "Prometheus operator 2026 kubernetes helm chart", "Grafana dashboards Dapr 2026 kubernetes". Likely shape: new `k8s/observability/` subdirectory with collector + prometheus + grafana manifests (or helm-chart values); Dapr Configuration extended with `tracing.zipkin.endpointAddress` (or OTLP) pointing at the in-cluster collector; new `cloud-observability-up` / `cloud-observability-down` Justfile recipes; new offline tests for the manifests; ADR-030 capturing decisions. Live cluster smoke (trace + metrics span propagation across cds-harness → cds-kernel) is Task 11.4's close-out gate.

> **Phase 1 axis 10 (FHIR) progress: 10.1 + 10.2 + 10.3 + 10.4 DONE — FHIR axis CLOSED.** **Phase 1 axis 11 (Cloud) progress: 11.1 + 11.2 DONE.** The strict §8.2 ordering rule selects `11.3` next. PHASE constants stay at 1 across Phase 1 and flip 1 → 2 at Task 12.4 close-out (ADR-024 §4).
>
> **ADR numbering drift (recorded for future sessions):** ADR-024 §6 pre-allocated ADR-026 → Task 11.1 and ADR-027 → Task 12.1. Actual landing has been sequential-by-task: ADR-026 → Task 10.3 (FHIRcast); ADR-027 → Task 10.4 (FHIR close-out); ADR-028 → Task 11.1 (cloud foundation); ADR-029 → Task 11.2 (cloud service deployment). Task 11.3 (observability) → ADR-030 expected; ZK toolchain (12.1) → ADR-031 expected. Sequential-by-task numbering is simpler than pre-reserved-by-axis; ADR-024 / ADR-028 pre-allocation notes remain planning intent only.

## Session 2026-05-04 — Task 11.2 close-out (ADR-029) — Cloud service deployment INTEGRATED

Closed the integration step of the Phase 1 cloud axis. ADR-028
explicitly carved image build + load + apply out of Task 11.1's
foundation scope (§"Pre-built container images already shipped at
11.1"). Task 11.2 ships the **mechanism** (Dockerfiles + lifecycle
recipes + offline tests); Task 11.4 ships the **end-to-end live
gate** (`contradictory-bound` UNSAT against kind). Task 11.3
(observability) lands between them so the close-out smoke can also
exercise the OpenTelemetry trace pipeline.

Three Dockerfiles, all multi-stage, all non-root `cds:10001`, all
exec-form ENTRYPOINT, all matching k8s manifest containerPort (8081 /
8082 / 3000). Image-tag convention `<app-id>:dev` matches the k8s
manifests' `imagePullPolicy: IfNotPresent` so `kind load docker-image`
resolves without a registry round-trip.

Six new Justfile recipes form the lifecycle:
`cloud-{build,load,up,down,status,smoke}`. Each gates on its required
tool with a loud notice on missing — same precedent as
`dapr-helm-install` / `kind-up` from 11.1.

`.dockerignore` at repo root keeps the build context trim. The
critical invariant (`.bin/z3` + `.bin/cvc5` MUST reach the cds-kernel
context) is asserted by `test_dockerignore_does_not_exclude_solver_bins`
which fails loudly if a future edit adds a blanket `.bin` / `.bin/*`
line.

ADR-029 §"Why slim, not distroless" documents the deliberate
deviation from the 2026 SOTA (Chainguard / distroless) that the
web-search surfaced — per-service curation cost outweighs the
~30MB-per-image saving for a research prototype. The Dockerfile shape
stays distroless-migration-ready (multi-stage, non-root, exec-form
ENTRYPOINT) for a Phase 2 reopener.

ADR-029 §"Why no Lean inside cds-kernel" documents the
deliberate-omission rationale (Kimina is external; HTTP-addressed via
`$CDS_KIMINA_URL`; in-cluster Kimina Service deployed at 11.4).
